...
-- Insert admin user manually (user will be created via auth UI)
-- This will be done through sign up form
...
-- Insert equipment units for each equipment
INSERT INTO public.equipment_units (equipment_id, unit_identifier, status) 
SELECT 
  e.id,
  'Unidade #' || generate_series(1, e.total_quantity),
  CASE 
    WHEN generate_series <= e.available_quantity THEN 'available'
    WHEN generate_series <= e.available_quantity + e.maintenance_quantity THEN 'maintenance'
    ELSE 'unavailable'
  END
