...
-- Step 2: Create security definer function to get current user role (fixes infinite recursion)
...
-- Step 3: Drop and recreate all profiles policies to fix infinite recursion
...
-- Step 4: Create restrictive policies for sensitive employee data
...
-- Step 5: Create restrictive policies for events (business critical data)
CREATE POLICY "Only admins and secretaries can view events"
ON public.events
FOR SELECT 
USING (public.get_current_user_role() IN ('admin', 'secretary'));

-- Step 6: Create restrictive policies for equipment data
...
-- Step 7: Enhance role validation trigger to prevent privilege escalation
